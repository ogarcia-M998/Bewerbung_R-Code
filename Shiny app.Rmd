...
# Summary
The following R.Markdown script sets up a Shiny app for visualizing data from the CSES Module 5 on national identity. The app allows to select a national identity variable (e.g., birthplace, language), a predictor (e.g., education, age), and filter by country. The app generates two plots: one showing the frequency distribution of the selected variable and another showing the effect of the chosen predictor on it, with a scatter plot and regression line to illustrate the relationship. The app combines an interactive UI and server-side logic to create these visualizations.
